...
//        @available(*, deprecated, message: "Use to instead")
    ...
    public struct ApproveOptions {
        ...
        public init(requesterAddress: String, approverAddress: String, privateKey: String, env: ENV) {
            fromDID = walletToPCAIP10(account: requesterAddress)
            toDID = walletToPCAIP10(account: approverAddress)
            self.privateKey = privateKey
            self.env = env

            isGroupChat = isGroupChatId(requesterAddress)

            if isGroupChatId(requesterAddress) {
                toDID = walletToPCAIP10(account: requesterAddress)
                fromDID = walletToPCAIP10(account: approverAddress)
            }
        }
    }
