...
extension PushChannel {
  ...
  public static func getSubscribers(option:GetChannelSubscribersOptions) async throws -> ChannelSubscribers{
    let _channel = walletToCAIPEth(account: option.channel, env: option.env)

    let url = try PushEndpoint.getSubscribers(channel: _channel, page: option.page, limit: option.limit, env: option.env).url
  ...
  public static func getChannels(option:GetChannelsOptions) async throws -> Channels{
    ...
    let url = try PushEndpoint.getChannels(page: option.page, limit: option.limit, env: option.env).url
