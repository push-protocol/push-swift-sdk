...
class ChatTests: XCTestCase {
    ...
    func testSendToGroup() async throws {
        ...
        let chats = try await userBob.chat.list(type: .CHAT)
        print(chats)

        let participants = try await userBob.chat.group.participants.list(chatId: group!.chatId)
        print("participants: \(String(describing: participants))")
    }
}
