...
class SearchChannelTests: XCTestCase {
  ...
  func testChannelByAddress() async throws {
    ...
    let res = try await PushChannel.search(
      option: PushChannel.SearchsOptions(query: channelAddress, page: 1, limit: 11, env: .STAGING))
  ...
  func testChannelByName() async throws {
    let query = "rayan"
    let res = try await PushChannel.search(
      option: PushChannel.SearchsOptions(query: query, page: 1, limit: 11, env: .STAGING))
